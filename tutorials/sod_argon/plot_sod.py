#!/usr/bin/env python3
"""
Argon Sod shock tube — compare SPLAY output against the exact Riemann solution.

Usage:
    python tutorials/sod_argon/plot_sod.py
    python tutorials/sod_argon/plot_sod.py --save sod_argon.png
    python tutorials/sod_argon/plot_sod.py --output_dir /path/to/output
"""
...
R_GAS  = 208.13       # J/(kg·K)  — specific gas constant for argon

# ── IC (must match argon_sod.yml) ────────────────────────────────────────────
...
def exact_riemann(rhoL, pL, uL, rhoR, pR, uR, gamma, x, t, x0):
    """
    Return (rho, u, p) arrays for the exact Riemann solution at time t.
    Handles left rarefaction / right shock (classic Sod topology).
    """
    gm1 = gamma - 1.0
    gp1 = gamma + 1.0
    ...
    # ── Star velocity (computed from the right wave to get the correct sign) ──
    # u* = uR + fR(p*):  fK_shock > 0 for right shock, so u* > uR = 0.
    # Equivalently from left: u* = uL - fL(p*), where fL < 0 for rarefaction.
    if p_star <= pR:
    ...
    if p_star <= pL:
        ...
        c_starL   = None  # not needed for shock

    # Right (shock if p* > pR — standard Sod)
    if p_star <= pR:
    ...
    for k in range(len(x)):
        ...
        if s <= S_HL:
            ...
            # Inside right rarefaction fan (uncommon for Sod)
    ...
    with open(path) as f:
        for line in f:
            ...
            if not line:
                continue
            if line.startswith("#"):
                # Parse time from header comment
...
# ── Main ──────────────────────────────────────────────────────────────────────
def main():
    parser = argparse.ArgumentParser(description="Argon Sod shock tube plot")
    parser.add_argument("--output_dir", default=DEFAULT_OUTPUT)
    parser.add_argument("--save", default="", help="Save figure to file")
    args = parser.parse_args()

    case_dir = os.path.join(args.output_dir, "argon_sod")
    csv_path = find_latest_csv(case_dir)
    if csv_path is None:
        print("No snapshot found. Run first:")
        print("  bash tutorials/sod_argon/run.sh --no-plot")
        sys.exit(1)
    ...
    fig.suptitle(
        f"Argon Sod shock tube — MUSCL+HLLC vs exact Riemann  (t={t_sim:.3e} s, N=400)",
    ...
    for ax, (key, ex_arr, ylabel) in zip(axes, panels):
        ...
        ax.plot(d["x"], d[key], "tab:blue", ls="--", lw=1.2, label="SPLAY MUSCL")
